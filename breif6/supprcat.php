<?php
 ...
 $connection = new PDO('mysql:host=localhost;dbname=breif6', $username, $password);
    $mysql = "SELECT * FROM `categories` WHERE cache = 1";
...
<?php
    ...
    $mysql1 = "SELECT * FROM `categories` WHERE cache = 0";
...
<div class="sidebar-sticky">
        ...
        <h2 class="mb-4 d-flex justify-content-center mt-3">MASK AND UNMASK A CATEGORY</h2>
...
<?php
// kanmaski categorys o ayi products liee bihoum
if (isset($_POST['mask'])) {
    $select = $_POST['selectOptions'];

    $updateQuery = "UPDATE categories SET cache = 0 WHERE categorie_id = '$select'";
...
if (isset($_POST['mask'])) {
    $select1 = $_POST['selectOptions'];
    $updateQuery1 = "UPDATE products SET cache_prod = 0 WHERE categorie_id = '$select1'";
    $updateStmt1 = $connection->prepare($updateQuery1);
    $updateStmt1->execute();
...
if (isset($_POST['rout'])) {
    ...
    $updateQuery = "UPDATE categories SET cache = 1 WHERE categorie_id = '$select'";
    $updateStmt = $connection->prepare($updateQuery);
    $updateStmt->execute();
    // header("Location: index.php");
    // exit;
...
if (isset($_POST['rout'])) {
    $select2 = $_POST['selectOptions'];
  

    $updateQuery2 = "UPDATE products SET cache_prod = 1 WHERE categorie_id = '$select2'";
    $updateStmt2 = $connection->prepare($updateQuery2);
    $updateStmt2->execute();
    // header("Location: index.php");
    // exit;
