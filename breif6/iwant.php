...
<div class="sidebar-sticky">
    ...
    <h2 class="mb-4 d-flex justify-content-center mt-3">USERS STATES</h2>
    <div class="row justify-content-center">
        <div class="col-md-6">
            <form method="post">
                <div class="form-group">
                    <label for="selectOptions1">To valid or remove a user:</label>
                    <select id="selectOptions1" name="selectOptions1" class="form-select">
                        <option value="">Here all accounts</option>
                ...
                <div class="d-flex justify-content-center">
                    <input type="submit" name="change" class="btn btn-primary mt-3 me-3" value="VALID">
                    <input type="submit" name="delete" class="btn btn-danger mt-3" value="Delete">
                </div>
            </form>
        </div>
        ...
        <div class="col-md-6">
            <form method="post">
                <div class="form-group">
                    <label for="selectOptions2">To make it admin:</label>
                    <select id="selectOptions2" name="selectOptions" class="form-select">
                        <option value="">Here all accounts</option>
                        <?php foreach ($users as $user) {
                            if ($user['admin'] == 0 && $user['states'] == 1) { ?>
                                <option value="<?= $user['id'] ?>"><?= $user['user_name'] ?></option>
                            <?php }
                        } ?>
                    </select>
                ...
                <div class="d-flex justify-content-center">
                    <input type="submit" name="exchange" class="btn btn-primary mt-3" value="VALID Admin">
                </div>
            </form>
        </div>
    </div>
...
<footer class="mt-2 bg-primary ">
        <div class="container">
            <div class="card bg-primary">
                <h5 class="bg-primary card-header">YOU FIND US AT  &nbsp; &nbsp; &nbsp;
                ...
                <div class="d-flex justify-content-between text-light bg-primary card-body">
                    ...
                    <div>
                        <h5>ABOUT</h5>
                        <p>We work in all cities of <br> Morocco</p>
                        <p>France</p>
                        <p>USA</p>
                        <p>Spain</p>
                    </div>
    ...
    <?php
    //bach kan9bel ula kandeleti user ola nredo admin 
    if (isset($_POST['change'])) {
        $userid = $_POST['selectOptions1'];
        $updatedata = "UPDATE users SET states = 1 WHERE id = :userid";
        $updateStmt = $connection->prepare($updatedata);
    ...
    if (isset($_POST['exchange'])) {
        $userid1 = $_POST['selectOptions'];
        $updatedata1 = "UPDATE users SET isadmin = 1 WHERE id = :userid1";
        $updateStmt = $connection->prepare($updatedata1);
        $updateStmt->bindParam(':userid1', $userid1);
        $updateStmt->execute();
 ...
 if (isset($_POST['delete'])) {
    $userid2 = $_POST['selectOptions1'];
    $sql = "DELETE FROM users WHERE	user_name = '$userid2' ";
