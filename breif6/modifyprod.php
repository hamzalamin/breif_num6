<?php
 ...
 // kanfetchi kolchi men tablau dyal product o kay7eto fdropdown dyalna
$mysqlProducts = "SELECT * FROM `products`";
$stmtProducts = $connection->prepare($mysqlProducts);
...
<section class="container mt-5 border border-primary rounded w-75 p-3 mb-5" style="background-color: rgba(26, 124, 209, 0.481);">
        <h2 class="mb-4 d-flex justify-content-center mt-3">MOdify A PRODUCT</h2>
        <div class="row justify-content-center">
            <div class="col-md-6">
                <form method="post">
                        ...
                        <option value="<?= $product['etiquette'] ?>"><?= $product['etiquette'] ?></option>
                    <?php } ?>
                                    </select>
                                </div>
                            </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="productPicture" name="pictureprod" required>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="form-group">
                    ...
                    <input type="submit" name="product" class="btn btn-primary mt-3 "  style="width: 10rem; margin-left: 35%;" value="UPDATE PRODUCT">
                </form>
            </div>
        </div>
    </section>
...
<footer class="mt-auto bg-primary">
  ...
  <path d="M12.6.75h2.454l-5.36 6.142L16 15.25h-4.937l-3.867-5.07-4.425 5.07H.316l5.733-6.57L0 .75h5.063l3.495 4.633L12.601.75Zm-.86 13.028h1.36L4.323 2.145H2.865l8.875 11.633Z"/>
    ...
    <?php
    // ila ladmin bgha i updatee 3la chi product
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        $select = $_POST['selectOptions'];
        $prodpic = $_POST['pictureprod'];
        $prodname = $_POST['etiquette'];
        $barre = $_POST['codebar'];
        $price1 = $_POST['pricedachat'];
        $price2 = $_POST['prixfin'];
        $price3 = $_POST['offre'];
        $proddisc = $_POST['productdiscription'];
        $qnt1 = $_POST['quantite_min'];
        $qnt2 = $_POST['quantite_stock'];

        try {
            $sql = "UPDATE products
                    SET image = :prodpic, etiquette = :prodname, code_barres = :barre, prix_dachat = :price1, prix_final = :price2, Offre_de_prix = :price3, description = :proddisc, quantite_min = :qnt1, quantite_stock = :qnt2
                    WHERE etiquette = :select";

            $stmt = $connection->prepare($sql);
            $stmt->bindParam(':prodpic', $prodpic);
            $stmt->bindParam(':prodname', $prodname);
            $stmt->bindParam(':barre', $barre);
            $stmt->bindParam(':price1', $price1);
            $stmt->bindParam(':price2', $price2);
            $stmt->bindParam(':price3', $price3);
            $stmt->bindParam(':proddisc', $proddisc);
            $stmt->bindParam(':qnt1', $qnt1);
            $stmt->bindParam(':qnt2', $qnt2);
